...
Description: Dependence of network organisation on overall FC
            -> classification: predict pcp's diagnosis (TC or ASD) based either
            on the graph measures 
...
Plan: - 1 - Libraries 
      ...
      - 5 - Features: network measures
      - 5.1 - FV - Param combinations
      - 5.2 - Training 
      - 5.3 - Select best params
      - 5.4 - Testing
...
# input variable
import sys
# var_iter = int(sys.argv[1])-1 # input - an index [0,36] indicating one of the pipeline combinations
...
# save_dir = "/Users/clarariegis/Desktop/Kings/Y3_2022_23/project/publication/results2"
os.chdir(data_dir)
...
# %%                - 6 - Features: network measures
# %%%               - 5.1 - FV - Param combinations
# FV - itertools to create parameter combinations
...
a = parallel_params_graph.values()
parallel_comb_graph = list(itertools.product(*a))
...
columns2 = ["roc_auc", "average_precision", "accuracy" , "recall" , "precision"]
...
for h in range(ncross):
    ...
    for i in range(nfolds):  # Looping through the 5 folds.
        ...
        # "reindex" part switched the ID columns to being "SUB_ID". It needs to be
       ...
       # –––––––––––––––––––– GRAPH MEASURES ––––––––––––––––––––
        ...
        this_threshold = parallel_comb_graph[var_iter][2]
    
        print(f"___________________ threshold : {this_threshold} ___________________")
        ...
        this_group = parallel_comb_graph[var_iter][1]
            
        print(f'___________________ standardization : {this_group} ___________________')
        
        x = train_folds_nr_gsr_mreg[this_group][list_plots]  
        
        
        
        #for this_kernel in kernels:
        this_kernel = parallel_comb_graph[var_iter][0]
            
        print(f'___________________ kernel : {this_kernel} ___________________')
        ...
        for this_c in c_params: 
            ...
            for this_gam in gam_params:
                ...
                if this_kernel == 'poly':
                    svc = SVC(kernel=this_kernel, C = this_c, gamma = this_gam, 
                ...
                if this_group == "group_mreg" : 
                    ...
                    val_pred = pipe_svm.predict(val_folds["group_pred"][list_plots]) 
                    # Proba 
                    val_prob = pipe_svm.predict_proba(val_folds["group_pred"][list_plots]) 
                    
                else:
                                          
                    # Prediction using the validation fold:
                    val_pred = pipe_svm.predict(val_folds[this_group][list_plots]) 
                    # Proba 
                    val_prob = pipe_svm.predict_proba(val_folds[this_group][list_plots])
                ...
                print("________ ONE PARAMETER ________")
    ...
    # Saving the results:
    #df_all_results.to_pickle("/Users/clarariegis/Desktop/Kings/Y3_2022_23/project/classif_results/train_meas_" + sample + "_fold_avr.pkl")
    # df_all_results.to_pickle(save_dir+"/graph/train_meas_" + str(var_iter) + "_fold_avr.pkl")
    df_all_results.to_pickle(save_dir+"/train_meas_" + sample + "_iter_" + str(var_iter) + "_crossval_" + str(h) + "_fold_avr.pkl")
...
## Retrieve the results (when spyder crash):
# path_files = save_dir+"/train_meas_" + sample + "_iter_" + str(var_iter) + "_crossval_*_fold_avr.pkl"
# results_list = []
# for filename in glob.glob(path_files):
#     results_list.append(pd.read_pickle(filename))
...
if this_group == "group_mreg" : 
    # Prediction using the validation fold:
    val_pred = pipe_svm.predict(test_folds["group_pred"][list_plots])
    # Proba 
    val_prob = pipe_svm.predict_proba(test_folds["group_pred"][list_plots])
    
else:
    # Prediction using the validation fold:
    val_pred = pipe_svm.predict(test_folds[this_group][list_plots])
    # Proba 
    val_prob = pipe_svm.predict_proba(test_folds[this_group][list_plots])
...
#df_all_results.to_pickle("/Users/clarariegis/Desktop/Kings/Y3_2022_23/project/classif_results/test_meas_" + sample + ".pkl")
df_test_results.to_pickle(save_dir+"/test_meas_" + sample + "_iter_" + str(var_iter) + ".pkl")


# %% load examples to check

# ex_meas_train = pd.read_pickle("~/Desktop/classify_out/train_edges_balanced_iter_0_fold_avr.pkl")
# ex_meas_test = pd.read_pickle("~/Desktop/classify_out/test_meas_balanced_iter_0.pkl")

# print(ex_meas_train.to_string())
# print(ex_meas_test.to_string())